...
function getRaceInfo() {
    let mode, laps, playerSpeedPercent, oCarSpeedLow, oCarSpeedTop, totalCars;
    let term, value;
    ...
    return {
        mode: mode, laps: laps, playerSpeedPercent: playerSpeedPercent, 
        oCarSpeedLow: oCarSpeedLow, oCarSpeedTop: oCarSpeedTop, totalCars: totalCars
    };
}
...
function getConfig() {
    ...
    let documentTitle = MODES[mode] + " - " + map.name + " - Retro Racing Game";
    if (oCarSpeedLow > oCarSpeedTop) { let spd = oCarSpeedLow; oCarSpeedLow = oCarSpeedTop; oCarSpeedTop = spd; }
    if (mode === "time_trial") { laps = 0; }

    let KEY = {
        LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40,
        W: 87, A: 65, S: 83, D: 68,
        C: 67, Q: 81, SPACE: 32, TAB: 9, ESC: 27
    };
    ...
    let sounds = {
        engine: {
            sound1: new Audio("./audio/engine1.mp3"),
            sound2: new Audio("./audio/engine2.mp3"),
            sound3: new Audio("./audio/engine3.mp3"),
            sound4: new Audio("./audio/engine4.mp3")
        },
    ...
    let rendering = {
        ...
        step: 0.4 / fps      // how long is each frame (in seconds)
    };
    ...
    let physics = {
        centrifugal: 0.5,    // centrifugal force multiplier when going around curves,
        maxDurability: 100
    };
    ...
    let camera = {
        ...
        fieldOfView: 140, // angle (degrees) for field of view
        height: 550,      // z height of camera
        ...
        cameraView: 2,    // current camera view (first person / third person)
        ...
        fogDensity: 2,    // exponential fog density
        motionBlurOn: true,
        screenShakeOn: true
    };
    ...
    let background = {
        image: null,
        skySpeed: 0.0001,    // background sky layer scroll speed when going around curve (or up hill)
        hillSpeed: 0.0002,   // background hill layer scroll speed when going around curve (or up hill)
        treeSpeed: 0.0003,   // background tree layer scroll speed when going around curve (or up hill)
    ...
    let road = {
        roadWidth: 3000,      // actually half the roads width, easier math if the road spans from -roadWidth to +roadWidth
        ...
        lanes: map.lanes,     // number of lanes,
        segments: []          // array of road segments
    }

    let speed = {
        current: 0,
        speedCap: segmentLength / step,          // speed cap (ensure we can't move more than 1 segment in a single frame to make collision detection easier)
        maxSpeed: speedCap * playerSpeedPercent
    };
    ...
    let player = {
        ...
        finishedPlace: -1,
        wrecked: false,
        accel: 3999,                 // acceleration rate - tuned until it 'felt' right
        breaking: -maxSpeed,         // deceleration rate when braking
        turning: 2,                  // turning speed
